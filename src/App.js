...
export default function App({ user, onLogout }) {
  ...
  useEffect(()=>{
    if(showLegal||showOnboarding||mainTab==="talk"||mainTab==="members"){
  ...
  },[showLegal,showOnboarding,mainTab]);
                ...
                {selectedEntry.image_url&&(()=>{
                  ...
                  return (
                    <div style={{position:"relative",marginBottom:12,borderRadius:12,overflow:"hidden",border:"1px solid rgba(212,175,55,.2)"}}>
                      ...
                      {/* Dot indicators */}
                      {imgs.length>1&&(
                        <div style={{display:"flex",justifyContent:"center",gap:5,padding:"8px 0 60px",background:"rgba(3,2,1,.6)"}}>
                          {imgs.map((_,i)=>(
